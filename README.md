...
> 【面试加分点】权限控制实现了 **4 层纵深防御**：① 路由守卫拦截 ② 侧边栏菜单动态过滤 ③ 子菜单点击前校验 ④ 具体按钮隐藏 + 点击拦截；每一层均返回「身份/范围/建议」3 段式提示。

---

## 🎬 面试 Show Script（3 条震撼路径）

### Script 1 · 权限系统深度展示（推荐给技术面）
1. admin 登录 → 账户管理页面 **可见**，设备/巡检编辑按钮 **可点击**
2. 退出 → inspector 登录 → 账户管理 **从侧边栏消失**；巡检记录的编辑/删除按钮 **隐藏**，点击会弹出「您是巡检人员身份，仅可新增巡检记录 → 如需编辑请联系管理员」3 段式提示
3. 退出 → user 登录 → 侧栏仅剩 3 个菜单，设备/巡检/系统设置全不可访问，路由守卫拦截并弹提示

### Script 2 · AI 一体化体验（推荐给产品/综合面）
1. 任意账号登录首页 → **居中 720px 巨型搜索栏**（仿 Chrome 新标签页）
2. 输入「帮我生成一份变压器季度巡检报告要点」 → 直接跳转全屏 AI Chat Page 自动发送问题
3. 返回首页 → 打开右下角悬浮 AI 助手，体验与全屏一致的对话 UI + `dot-pulse` 思考动画

### Script 3 · 数据大屏 + 代码质量（推荐给技术 Leader）
1. 进入「数据统计」页面 → ECharts 4 种图表同屏：设备类型饼图 / 月度巡检趋势折线 / 告警分布柱状 / 健康度仪表盘
2. F12 → Network 观察 chunk 请求：`vue-vendor` / `element-plus-vendor` / `echarts-vendor` 独立缓存
3. F12 → Console 输入 `document.querySelectorAll('[style*="transition: all"]').length` 返回 0 —— 证明全项目已清除性能 antipattern
